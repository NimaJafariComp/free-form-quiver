...
export const freeform_svg = (scene, { padding = 24, background = null } = {}) => {
    ...
    const node_svg = [...nodes.values()].map((node) => {
        ...
        return `<g class="qv-node"><rect x="${number(node.bounds.x)}" y="${number(node.bounds.y)}" width="${number(node.bounds.width)}" height="${number(node.bounds.height)}" rx="${number(frame.radius || 14)}" fill="${frame.background || "#d0d0d0"}" stroke="${frame.border || "none"}"/><foreignObject x="${number(node.bounds.x)}" y="${number(node.bounds.y)}" width="${number(node.bounds.width)}" height="${number(node.bounds.height)}"><div xmlns="http://www.w3.org/1999/xhtml" class="qv-node-label" style="color:${node.colour || "#111"}">${label}</div></foreignObject></g>`;
    }).join("");
